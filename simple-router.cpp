        ...
        if(e_type==ethertype_arp){
            ...
            if(o_code==1){
                ...
                if(etmac==broadcastMac&&a_header.arp_tip==iface->ip){
                    ...
                    //construct a ethernet header for to send
            ...
            if(externalInterface ==  nullptr )
                cerr << "external interface is null" << endl;
           // if(ipToString(ipDest->ip).compare(ipToString(externalInterface->ip)) != 0)
             //   cerr << "same ip dest and external int" << endl;ipToString(ipDest->ip).
            //cerr << "IPDEST" <<ipToString(ipDest->ip)<< endl;
            if ((nat_flag == 0 && externalInterface ==  nullptr && ipDest != nullptr) || (ipDest != nullptr && ipToString(ipDest->ip).compare(ipToString(externalInterface->ip)) != 0)) {
                //to this router, deal with it
                cerr << "ip_header.ip_p->"<<ipToString(ip_header.ip_p) << endl;
                if(ip_header.ip_p == 1){
                    cerr<<"Its icmp"<<endl;
                    ...
                    //error
                    cerr<<"Meant for this router but no icmp, so drop it"<<endl;
                    return;
                }
            }
            else{
                ...
                if(nat_flag == 1){
                    ...
                    struct icmp_hdr ic_header;
                    memcpy(&ic_header, &(packet[sizeof(ethernet_hdr)+sizeof(ip_hdr)]), sizeof(icmp_hdr));
                    //uint16_t id = ic_header.icmp_id;
                    if (ic_header.icmp_type == 8) { //echo request forwarded to sw0-eth1, why??????
                        cerr<<"NAT: ECHO REQUEST"  << endl;
                        cerr<<"ID-"<<ic_header.icmp_id<< endl;
                        uint32_t src  = ip_header.ip_src;
                        const Interface* externalInterface = findIfaceByName("sw0-eth4");
                        cerr<<"IP source b4"  << ipToString(ipHder->ip_src) << endl;
                        ipHder->ip_src = externalInterface->ip; //change header to external ip address
                        cerr<<"IP source after"  << ipToString(ipHder->ip_src) << endl;
                        cerr<<"IP dst "  << ipToString(ipHder->ip_dst) << endl;
                        m_natTable.insertNatEntry(ic_header.icmp_id, src, externalInterface->ip); //check if in nat already
                    }
                    else if(ic_header.icmp_type == 0){
                        cerr<<"NAT: ECHO REPLY" << endl;
                        //uint16_t id = ip_header.ip_id;
                        cerr<<"ID-"<<ic_header.icmp_id << endl;
                        std::shared_ptr<NatEntry> entry = m_natTable.lookup(ic_header.icmp_id);
                        ipHder->ip_dst = entry->internal_ip; //issue here!q
                        ip_header.ip_dst = entry->internal_ip;
                        cerr<<"IP source "  << ipToString(ipHder->ip_src) << endl;
                        cerr<<"IP dest "  << ipToString(ipHder->ip_dst) << endl;
                    }
                }
                ...
                if (arpEntry != nullptr) {
                    ...
                    uint8_t* sendPack = (uint8_t*)s_packet.data();
                    ethernet_hdr* e_header = (ethernet_hdr*)sendPack;
                    ip_hdr* ip_header = (ip_hdr*)(sendPack + sizeof(ethernet_hdr)); //different ip
                    ...
                    cerr<<"IP source "  << ipToString(ip_header->ip_src) << endl;
                    cerr<<"IP dest "  << ipToString(ip_header->ip_dst) << endl;
